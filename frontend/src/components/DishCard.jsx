...
export default function DishCard({ dish, isOwner, onEdit, onDelete, onToggleAvailability }) {
  ...
  const isAvailable = dish?.isAvailable !== false ?? true;

  return (
    <div className={`dish-card ${!isAvailable ? "unavailable" : ""}`}>
      <div className="dish-image-container">
        <img src={getImageUrl(dish?.image)} alt={dish?.name || 'Dish'} className="dish-image" />
        <span className="dish-category">{dish?.category || 'Uncategorized'}</span>
        {!isAvailable && (
      ...
      <div className="dish-content">
        <h3 className="dish-name">{dish?.name || 'Unnamed Dish'}</h3>
        ...
        <div className="dish-footer">
          ...
          {isCustomer && isAvailable && (
            <button className="add-to-cart-btn" onClick={() => addToCart(dish)} title="Add to cart" disabled={!dish}>
          ...
          {isOwner && (
            <div className="dish-actions">
              <button 
                ...
                onClick={() => dish?._id && onToggleAvailability(dish._id)}
              ...
              <button className="edit-btn" onClick={() => dish && onEdit(dish)} disabled={!dish}>
                <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  <path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/>
                </svg>
                <span>Edit</span>
              </button>
              <button className="delete-btn" onClick={() => dish?._id && onDelete(dish._id)} disabled={!dish}>
                <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
